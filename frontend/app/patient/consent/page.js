...
export default function PatientConsentPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const consent = await getPatientConsent(address);
    ...
    loadData();
  }, [signer, address, getPatientConsent, getPaymentTerms, checkVerificationStatus]);
  ...
  const handleSubmit = async (formData) => {
    ...
    const res = await submitPatientConsent(formData);
    setResult(res);
      ...
      <div className="max-w-2xl mx-auto py-12 px-4">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 dark:text-gray-400">
            You have already completed the patient consent form.
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg mb-8">
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex justify-between">
              <span className="text-gray-500">Aggregate Queries:</span>
              <span>{existingConsent.acceptsAggregateQueries ? '✓ Allowed' : '✗ Not Allowed'}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">Individual Queries:</span>
              <span>{existingConsent.acceptsIndividualQueries ? '✓ Allowed' : '✗ Not Allowed'}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">International Research:</span>
              <span>{existingConsent.allowsInternationalResearch ? '✓ Allowed' : '✗ Not Allowed'}</span>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="max-w-3xl mx-auto py-12 px-4">
      ...
      {step === 'worldid' && (
        <div className="space-y-6">
          <div className="text-center mb-6">
            ...
            <p className="text-gray-600 dark:text-gray-400">
              World ID verification is optional but provides enhanced trust on the platform
            </p>
          </div>
      ...
      {step === 'consent' && (
        <div className="space-y-6">
          <div className="text-center mb-6">
            <h1 className="text-3xl font-bold mb-2">Patient Consent Form</h1>
            <p className="text-gray-600 dark:text-gray-400">
              Review and agree to the terms for contributing your health data
            </p>
          </div>
